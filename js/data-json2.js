...
xhr.onload = function() {                       // When readystate changes
  ...
  //if(xhr.status === 200) {                      // If server status was ok
    ...
    for (var i = 0; i < responseObject.events.length; i++) { // Loop through object
      ...
      newContent += '<img src="' + responseObject.events[i].map + '" width = 250 height = 200 ';
    ...
    document.getElementById('contentTwo').innerHTML = newContent;

  //}
};

xhr.open('GET', 'data/data2.json', true);        // Prepare the request
